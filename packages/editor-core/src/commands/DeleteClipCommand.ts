...
interface DeleteClipCommandPayload {
   clipId: string
}

export function createDeleteClipCommand(payload: DeleteClipCommandPayload): Command {
   ...
   return {
      ...
      execute(project: Project): Result<Project, EditorError> {
         ...
         const newTracks = project.timeline.tracks.map(t =>
            t.id === track.id ? { ...t, clips: t.clips.filter(c => c.id !== payload.clipId) } : t
         )
