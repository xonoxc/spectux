...
export function createFFmpegRenderer(): Renderer {
  ...
  async function collectAssets(
    project: Project,
  ): Promise<Result<ExportAsset[], EditorError>> {
    const assetFiles = new Map(
      project.assets.map((asset) => [asset.id, asset.fileName]),
    )
  ...
  return {
    ...
    async export(project) {
      ...
      if (renderRes.isErr()) return ffmpegError(renderRes.error)
    ...
    async cancel() {
      const cancelRes = workerInstance
        ? await attempt(workerInstance.cancel())
        : null
