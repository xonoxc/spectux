...
export function InspectorPanel() {
   ...
   const selectedClip =
      selectedClipIds.length === 1 ? findClip(project.timeline, selectedClipIds[0]) : null
      ...
      <div className="flex h-full flex-col">
         ...
         <div className="flex-1 overflow-y-auto p-3">
            ...
            {!inspectedAsset && selectedClip?.isOk() && (
               <ClipProperties clip={selectedClip.value.clip} track={selectedClip.value.track} />
            )}
         </div>
      </div>
   )
   ...
   return (
      <div className="space-y-3">
         ...
         <Section title="Timing">
            <Property label="Timeline Start" value={`${clip.timelineStart.toFixed(2)}s`} />
            <Property label="Source Start" value={`${clip.start.toFixed(2)}s`} />
            <Property label="Source End" value={`${clip.end.toFixed(2)}s`} />
            <Property label="Duration" value={`${(clip.end - clip.start).toFixed(2)}s`} />
         </Section>
      </div>
   )
}
...
function Property({ label, value }: { label: string; value: string }) {
   return (
      <div className="flex items-center justify-between">
         ...
         <span className="max-w-35 truncate font-mono text-xs text-neutral-300">{value}</span>
      </div>
   )
}
