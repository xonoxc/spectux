...
export async function saveAssetBlob(asset: Asset, projectId: string, blob: Blob): Promise<void> {
   await db.assets.put({
      id: asset.id,
      projectId,
      name: asset.name,
...
export async function listProjectAssets(projectId: string): Promise<AssetRecord[]> {
   return db.assets.where("projectId").equals(projectId).reverse().sortBy("importedAt")
}
