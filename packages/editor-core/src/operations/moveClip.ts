...
export function moveClipToTime(
   ...
   const updatedClips = [...otherClips, movedClip].sort((a, b) => a.timelineStart - b.timelineStart)
...
export function moveClipToTrack(
   ...
   const targetTrack = project.timeline.tracks.find(t => t.id === targetTrackId)
   if (!targetTrack) {
      return err({ type: "TIMELINE.TRACK_NOT_FOUND", trackId: targetTrackId })
   }
   ...
   const newTracks = project.timeline.tracks.map(t => {
      ...
      if (t.id === targetTrackId) {
         return {
            ...t,
            clips: [...t.clips, movedClip].sort((a, b) => a.timelineStart - b.timelineStart),
         }
      }
      return t
   })
