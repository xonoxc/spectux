...
interface ChangeVolumeCommandPayload {
   clipId: string
   volume: number
}

export function createChangeVolumeCommand(payload: ChangeVolumeCommandPayload): Command {
               ...
               ? {
                    ...
                    clips: t.clips.map(c =>
                       c.id === payload.clipId
                          ? {
                               ...c,
                               volume: Math.max(0, Math.min(2, payload.volume)),
                            }
                          : c
                    ),
                 }
