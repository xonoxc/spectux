...
export function findSegment(segments: ClipSegment[], time: number): ClipSegment | null {
...
export function getClipAtTime(segments: ClipSegment[], time: number): ClipSegment | null {
   return (
      segments.find(segment => time >= segment.timelineStart && time < clipTimelineEnd(segment)) ??
      null
   )
}

export function getActiveAudioClips(segments: ClipSegment[], time: number): ClipSegment[] {
   return segments.filter(
      s => s.type === "audio" && time >= s.timelineStart && time < clipTimelineEnd(s)
   )
}

export function getNextClipAfterTime(segments: ClipSegment[], time: number): ClipSegment | null {
   return (
...
export function assetToTimelineTime(seg: ClipSegment, assetTime: number): number {
   return seg.timelineStart + (assetTime - seg.assetStart)
}

export function timelineToAssetTime(seg: ClipSegment, timelineTime: number): number {
...
export class TimelinePlayer {
   ...
   private tick = (frameTime: number) => {
      ...
      if (currentVideoSegment && this.timelineTime >= clipTimelineEnd(currentVideoSegment)) {
