...
export function removeTrack(timeline: Timeline, trackId: string): Result<Timeline, EditorError> {
...
export function getTrack(timeline: Timeline, trackId: string): Result<Track, EditorError> {
...
export function totalDuration(timeline: Timeline): number {
   ...
   return Math.max(
      ...timeline.tracks.map(t => {
         ...
         return Math.max(...t.clips.map(c => c.timelineStart + (c.end - c.start)))
      })
   )
}
